...
(defn cached-service
  "Returns a service that subscribes each identifier at service only
  once and caches the deltas.  opts supports the following keys

  :mechanism - either :deltas or :sum.  :deltas caches all deltas and
  streams them into each new subscription.  :sum only caches the
  current sum and streams it as an :is delta into a new subscription.
  Defaults to :sum.

  :mechanisms - a map identifier->mechanism for individual
  ...
  (let [cache (atom {})]
    (reify
      ...
      (subscribe [_ identifier]
        ;; 1. Atomically prepare or return cache for identifier and
        ;; determine whether it needs to be subscribed
        
        ;; 2. If it exists, put onto return channel init delta, then
        ;; continue with deltas from subscription

        ;; 3. Otherwise attach return channel, then subscribe it.

        ;; Trap: deltas could be lost in (2).

        ;; Solution: Implement version of mult that "greets" new
        ;; channels with an init-delta, if there is none, it will just
        ;; transport incoming deltas.
        )
      (cancel [_ ch]))))
